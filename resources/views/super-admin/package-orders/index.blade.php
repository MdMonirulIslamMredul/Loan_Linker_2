...
<div class="card border-0 shadow-sm">
    <div class="card-body p-0">
        <div class="p-3 border-bottom bg-white shadow-sm rounded-bottom-0">
            <form method="GET" class="row g-2">
                ...
                <div class="col-md-3 position-relative">
                    ...
                    <input type="search" id="officer-search" name="search" class="form-control"
                        value="{{ request('search') }}" placeholder="Search by name, email or phone" autocomplete="off">
                    <input type="hidden" id="officer-id" name="officer_id" value="{{ request('officer_id') }}">
                    <div id="officer-search-results"
                        class="list-group position-absolute w-100 shadow-sm bg-white rounded overflow-auto"
        ...
        <div class="col-md-12 text-end mt-2">
            ...
            <a href="{{ route('super-admin.package-orders.print', request()->query()) }}" target="_blank"
                class="btn btn-outline-dark btn-sm ms-1">Print</a>
        ...
        <table class="table table-hover table-borderless align-middle mb-0">
            ...
            <tbody>
                ...
                <tr
                    ...
                    <td>
                        <a href="#" class="user-info btn btn-sm btn-outline-success text-start"
                            data-name="{{ e($order->user->name) }}" data-email="{{ e($order->user->email) }}"
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function renderOfficerSearchResults(results) {
            ...
            if (!results.length) {
                officerSearchResults.innerHTML =
                    '<div class="list-group-item small text-muted">No matching branch admin found.</div>';
                officerSearchResults.style.display = 'block';
                return;
            }
            officerSearchResults.innerHTML = results.map(function(user) {
                return '<div class="list-group-item list-group-item-action text-start" role="button" data-officer-id="' +
                    user.id + '" data-officer-email="' + user.email + '">' +
            ...
            officerSearchResults.querySelectorAll('[data-officer-id]').forEach(function(item) {
                item.addEventListener('click', function() {
                    ...
                    if (officerSearchInput) {
                        officerSearchInput.value = this.dataset.officerEmail ||
        ...
        if (officerSearchInput) {
            ...
            document.addEventListener('click', function(event) {
                if (officerSearchResults && !officerSearchResults.contains(event.target) && event
                    .target !== officerSearchInput) {
                    officerSearchResults.style.display = 'none';
                }
            });
        }
